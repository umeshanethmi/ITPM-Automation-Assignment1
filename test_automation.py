...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "IT23861718_Assignment1.xlsx"),
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                if action_locator:
                    ...
                    try:
                        page.wait_for_selector('button:not([disabled]):has-text("Transliterate")', timeout=20000)
                    except Exception:
                        pass
                ...
                for i in range(tries):
                    ...
                    if not current:
                        ...
                        continue
                    if prev_output and current == prev_output and singlish_input != prev_input:
                        page.wait_for_timeout(max(0, int(args.retry_wait_ms)))
                        continue
                    if current:
                ...
                if prev_output and actual_output == "" and prev_output != "" and singlish_input != prev_input:
                    print("Warning: Output did not update for this input (still showing previous or empty output).")
                elif actual_output == "" and prev_output and singlish_input == prev_input:
                    actual_output = prev_output

                prev_input = singlish_input
                ...
                import traceback
                print(f"Error in UI interaction:")
                traceback.print_exc()
                try:
